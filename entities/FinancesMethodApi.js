...
async function getFinances() {
  const finances = await fetch("https://apifinances.vercel.app/finances");
  return finances.json();
};

/* API POST */
async function responsePOST(objectInput) {
  await (await fetch("https://apifinances.vercel.app/finances", {
    method: "POST",
    headers: {
...
async function responsePUT(objectId, objectInput) {
  await (await fetch(`https://apifinances.vercel.app/finances/${objectId}`, {
    method: "PUT",
    headers: {
      "Content-Type": "application/json"
    },
